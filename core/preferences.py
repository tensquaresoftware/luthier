...
_WORKSPACE_KEYS = WORKSPACE_KEYS
...
def factory_defaults(desktop: str | None = None) -> dict:
    """Return a fresh profile dict with factory values (Desktop on host OS)."""
    data = dict(_DEFAULTS)
    if desktop is None:
    ...
    data[host_dest] = normalize_portable_path(desktop)
    return data
...
def _complete_profile(data: dict) -> dict:
    ...
    migrated = normalize_path_dict_values(data)
    merged = {key: migrated.get(key, _DEFAULTS.get(key)) for key in _PROFILE_KEYS}
...
def validate_profile(
    ...
    *,
    ...
    for key in WORKSPACE_KEYS:
        ...
        if value.strip():
            ok, message = validation.validate_optional_path(value)
            if not ok:
                return False, message
    checks_text = [
    ...
    for key, validator in checks_text:
        ok, message = validator(str(profile.get(key, "")))
        if not ok:
            return False, message
    plugin_type = str(profile.get("pluginType", ""))
    if plugin_type not in _VALID_PLUGIN_TYPES:
        return False, (
            "Plugin type must be instrument, audio-effect, or midi-effect."
        )
    formats = str(profile.get("pluginFormats", "")).split()
    if not formats:
    ...
    cxx = str(profile.get("cxxStandard", ""))
...
class Preferences:
    ...
    def save(self) -> None:
        payload = {key: self.get(key) for key in _PROFILE_KEYS}
        payload["accentColor"] = self.accent_color
    ...
    def host_juce_dir(self) -> str:
        return str(self.get(host_workspace_field_key("juce")) or "").strip()
    ...
    def apply_form(self, identity: dict, workspace: dict, artefacts: dict) -> None:
